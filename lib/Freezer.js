...
function createFoodTable(users) {
    ...
    for (var i = 0; i < users.length; i++) {
        ...
        var stockCell = newElem('th', 'UserCell Checked');
        stockCell.innerHTML = users[i];
        stockCell.id = 'UserCell' + i;
...
function appendStocks(row, food, users) {
    for (var j = 0; j < users.length; j++) {
        ...
        appendCell(row).appendChild(stockField);
    }
 ...
 * the query.
 * If no all users are selected in the table, a list of selected users will also
 * be included.
 * 
 * @param {array(string)} users
 */
function foodSearch(users) {
    ...
    if (isSet(getElem('foodTable'))) {
        ...
        // Add user list if not all selected
