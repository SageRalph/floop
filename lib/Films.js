...
function createFilmTable(users) {
    ...
    var html = "<th class='TrailerCell'>Trailer</th> \
                <th class='TitleCell'>Title</th>";

    // User name column headers
    for (var i = 0; i < users.length; i++) {
        log("Adding user to table: " + users[i]);
        html += "<th class='RatingCell'>" + users[i] +
                "<input type='checkbox' class='UserCheckbox' checked \
                onchange='manageFilms()' id='RatingCell" + i + "'></input></th>";
    }

    html += "<th class='TotalRatingCell'>Total</th>";
    ...
    var toggleEditBtn = newBtn("Edit All", function () {
        alwaysEditingFilms = !alwaysEditingFilms;
        toggleEditBtn.value = alwaysEditingFilms ? "Edit Off" : "Edit All";
        manageFilms();
    });
    appendBtnCell(row, toggleEditBtn);
...
function editFilm(users, row, film) {
    ...
    if (alwaysEditingFilms) {
        ...
        if (isSet(film.trailer)) {
            ...
            trailerCell.appendChild(newBtn("Search", function () {
